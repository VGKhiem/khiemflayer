...
const BlockFaces = require('khiemflayer-minecraft-world').iterators.BlockFace
...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    try {
      ...
      waitTimeout = setTimeout(finishDigging, waitTime + 50)
      ...
      function finishDigging () {
        ...
        waitTimeout = setTimeout(() => {
          ...
          diggingTask.cancel(new Error('Digging timed out waiting for server block update'))
        }, 2500)
      }
      ...
      function onBlockUpdate (oldBlock, newBlock) {
        ...
        if (newBlock.type !== 0 && newBlock.type === currentBlock.type) {
          ...
          if (elapsed > 500) { // Server reset the block (likely rejected finish)
            bot.removeListener(eventName, onBlockUpdate)
            clearInterval(swingInterval)
            clearTimeout(waitTimeout)
            swingInterval = null
            ...
            bot.stopDigging = noop
            diggingStartedAt = null
            diggingTask.cancel(new Error('Server reset block digging progress'))
          }
          return
        }
        bot.removeListener(eventName, onBlockUpdate)
