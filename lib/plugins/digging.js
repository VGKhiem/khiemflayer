...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    function startDigAttempt () {
      if (!bot.targetDigBlock) return // dig was already cancelled or completed
      digRetryCount++
      ...
      bot._client.write('block_dig', {
        ...
        sequence: digSequence++
      })
      waitTimeout = setTimeout(finishDigging, waitTime + 250 * digRetryCount)
    ...
    function finishDigging () {
      ...
      // Wait for server confirmation via block update.
      // If no update arrives within 1s, check block state and retry if needed.
      waitTimeout = setTimeout(() => {
        // If dig was already resolved by onBlockUpdate, bail out
        if (!bot.targetDigBlock) return
        const currentBlockAtPos = bot.blockAt(block.position)
        if (currentBlockAtPos && currentBlockAtPos.type === 0) {
          // Block is actually broken, just missed the update packet
          ...
          diggingStartedAt = null
          bot.stopDigging = noop
          bot.emit('diggingCompleted', currentBlockAtPos)
          diggingTask.finish()
        } else if (bot.targetDigBlock) {
          // Server didn't respond or rejected - retry the dig
          startDigAttempt()
        }
      }, 1000)
    }
    ...
    function onBlockUpdate (oldBlock, newBlock) {
      if (!newBlock) return
      if (newBlock.type !== 0 && newBlock.type === currentBlock.type) {
        // Server reset the block (likely rejected finish) - retry
        // Only retry if enough time has passed to avoid false triggers on start sync
