...
function inject (bot) {
  ...
  const armorKey = (bot.registry && bot.registry.isNewerOrEqualTo && bot.registry.isNewerOrEqualTo('1.20.5') ? 'minecraft:armor' : 'generic.armor')
  const armorToughnessKey = (bot.registry && bot.registry.isNewerOrEqualTo && bot.registry.isNewerOrEqualTo('1.20.5') ? 'minecraft:armor_toughness' : (bot.registry && bot.registry.isNewerOrEqualTo && bot.registry.isNewerOrEqualTo('1.16') ? 'generic.armor_toughness' : 'generic.armorToughness'))
  ...
  bot.getExplosionDamages = (targetEntity, sourcePos, power, rawDamages = false) => {
    ...
    if (!rawDamages && targetEntity.attributes[armorKey]) {
      const armor = getAttributeValue(targetEntity.attributes[armorKey])
    ...
    } else if (!rawDamages && !targetEntity.attributes[armorKey]) {
      return null
    }
