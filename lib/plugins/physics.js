...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  const POSITION_EVERY_N_TICKS = (bot.registry && bot.registry.isNewerOrEqualTo && bot.registry.isNewerOrEqualTo('1.21.5') ? 19 : 20)
  ...
  function tickPhysics (now) {
    ...
    if (bot.version === '1.21.5' || (bot.registry && bot.registry.isNewerOrEqualTo && bot.registry.isNewerOrEqualTo('1.21.3'))) {
  ...
  function sendPacketPosition (position, onGround) {
    ...
    lastSent.flags = { onGround, hasHorizontalCollision: false } // 1.21.3+
  ...
  function sendPacketVehicleMove (position) {
    bot.vehicleMove = bot.vehicleMove ?? {
  ...
  function updatePosition (now) {
    ...
    if (onVehicle) {
      ...
      bot._client.write('flying', {
        ...
        flags: { onGround: bot.entity.onGround, hasHorizontalCollision: false } // 1.21.3+
      })
    }
