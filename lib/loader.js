...
function createBot (options = {}) {
  if (options.viaProxy || options.forceViaProxy) {
    throw new Error('To use viaProxy, you must use createBotAsync() instead of createBot() because it requires asynchronous setup.')
  }
...
async function createBotAsync (options = {}) {
  const viaproxy = require('mineflayer-viaproxy')
  return await viaproxy.createBot(options, createBot)
}
